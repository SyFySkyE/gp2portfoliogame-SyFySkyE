...
public static class Matching2
{
    ...
    public static List<SinglePiece> GetConnectedPieces(SinglePiece currentPiece, SinglePiece[,] pieceArray, Vector2[] directionsToCheck, bool includeCorners)
    {
        ...
        foreach(Vector2 dir in directionsToCheck)
        {
            ...
            {
                if (includeCorners) // Checks for L matches
                {
                    if (dir.y != 0)
                    {
                        if (CheckInitialMatch(currentPiece, pieceArray, Directions.UpDirection))
                        {
                            foreach (SinglePiece piece in GetConnectedPieces(currentPiece, pieceArray, Directions.UpDirection, false))
                            {
                                connectedPieces.Add(piece);
                            }
                        }                        
                    ...
                    {
                        if (CheckInitialMatch(currentPiece, pieceArray, Directions.RightDirection))
                        {
                            foreach (SinglePiece piece in GetConnectedPieces(currentPiece, pieceArray, Directions.RightDirection, false))
                            {
                                connectedPieces.Add(piece);
                            }
                        }
    ...
    //private void CheckForLMatches(SinglePiece currentPiece, List<SinglePiece> connectedPieces, Vector2 matchingDir)
    //{
    //    Vector2 dirToCheck;
    //    if (matchingDir.x != 0)
    //    {
    //        dirToCheck = Vector2.up;
    //    }
    //    else
    //    {
    //        dirToCheck = Vector2.right;
    //    }
    //}
}
